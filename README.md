...
The IDS dll is a light-weight library, that allows any IDS to be encoded and decoded to binary representation (as a char/byte array).
The library supports  signed and unsigned numbers and fraction numbers.

Example for conversion from an IDS to binary representation:
```cpp
...
Example for conversion from a binary representation to an IDS:
```cpp
