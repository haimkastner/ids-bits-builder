...
public:
    ...
    /**
     ...
     @param binaryArray The raw binary data as chars array to convert from.
     @return A loaded fildes array with the converted value.
     */
    static std::vector<Field> decode_ids(IDS idsFields, std::vector<char> binaryArray);

    /**
     Load the IDS fileds with data from a raw binary data.
     @return A loaded fildes array with the converted value.
     */
    std::vector<Field> decode_ids();
